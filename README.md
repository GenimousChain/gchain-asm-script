...
<<<<<<< HEAD
<<<<<<< HEAD
All the details are provided in the [AssemblyScript wiki](https://github.com/GenimousChain/assemblyscript/wiki) - make sure to pay it a visit. With that being said, the easiest way to get started with AssemblyScript is to point npm at the GitHub repository (for now)

```
$> npm install --save-dev GenimousChain/assemblyscript
```

followed by [scaffolding](https://github.com/GenimousChain/assemblyscript/wiki/Using-the-CLI#scaffolding-with-asinit) a new project including the necessary configuration files, for example in the current directory:

```
$> npx asinit .
```

Once the project is set up, it's just a matter of using your existing [TypeScript tooling](https://code.visualstudio.com) while coding, and [using the CLI](https://github.com/GenimousChain/assemblyscript/wiki/Using-the-CLI) to build to WebAssembly, either manually, or using (and maybe modifying) the generated build task in the generated `package.json`:
...
```
$> git clone https://github.com/GenimousChain/assemblyscript.git
$> cd assemblyscript
$> npm install
$> npm link
$> npm clean
